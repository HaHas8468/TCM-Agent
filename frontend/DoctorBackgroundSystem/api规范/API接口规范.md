...
响应：
{
  "code": 0,
  "data": [
    { "type": "diagnosis_ready", "title": "您的诊断已生成", "order_id": "O20260710001", "time": "2026-07-10T10:30:00" }
  ]
}
...
| 7 | /api/departments | GET | 科室+医生列表 |
| 8 | /api/doctors/{id}/slots | GET | 医生可预约时段 |
| 9 | /api/orders | POST | 创建挂号单 |
| 10 | /api/doctor/queue | GET | 接诊队列 |
| 11 | /api/orders/{id}/start | PUT | 开始接诊 |
| 12 | /api/orders/{id}/finish | PUT | 完成接诊 |
| 13 | /api/orders/{id}/save | PUT | 暂存 |
| 14 | /api/orders/{id}/diagnosis | POST | 提交四诊信息 |
| 15 | /api/records | GET | 病历检索 |
| 16 | /api/records/{id} | GET | 病历详情 |
| 17 | /api/patient/{id}/history | GET | 患者历史病历 |
| 18 | /api/patients/bypatientid/{id}/basic | GET | 患者基本信息（按业务号） |
| 19 | /api/patient/diagnosis-history | GET | 历史诊断列表 |
| 20 | /api/orders/{id} | GET | 挂号/病历详情 |
| 21 | /api/patient/latest-diagnosis | GET | 最近一次诊断 |
| 22 | /api/doctor/profile | GET/PUT | 医生资料 |
| 23 | /api/admin/users | GET | 用户管理 |
| 24 | /api/patient/notifications | GET | 消息通知 |
...
E.3 接口调用的 4 种状态（data.status）
| status | 含义 | 前端处理 |
|--------|------|---------|
| asking | 信息不完整，需要继续追问 | 渲染追问气泡，保留用户上下文，等待下一轮 |
| diagnosed | 诊断完成 | 渲染诊断卡片（辨证/方剂/药材/科室/过敏警告） |
| done | 自定义查询/医案| 渲染纯文本回答 |
| error | 处理失败 | 显示"系统繁忙请重试"，可重发 |
